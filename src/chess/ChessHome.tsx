...
import { useGame, type MoveResult } from "@/hooks/useGame";
...
export default function ChessHome() {
  ...
  useEffect(() => {
    if (gameId && userToken) {
      const websocket = new WebSocket(
        `${WS_BASE_URL}/ws/chess/game/${gameId}/?token=${userToken}`,
      );
      websocket.onopen = () =>
      ...
      websocket.onmessage = (event) => {
        ...
        if (data.type === "game_update") {
          if (data.fen) gameActions.syncFromFen(data.fen);
        }
      };
    ...
    const checkForMatch = async () => {
      ...
      try {
        ...
        const activeGame = Array.isArray(games)
          ? games.find((g: any) => g.status === "active")
          : null;
        if (activeGame) {
  ...
  const startGame = async () => {
    ...
    if (opponent === "bot") {
      try {
        const response = await fetch(
          `${API_BASE_URL}/chess/games/start_bot_game/`,
          {
            method: "POST",
            headers: {
            ...
            body: JSON.stringify({ stake: 0, bot_difficulty: "medium" }),
          },
        );
        const data = await response.json();
        if (response.ok && data.game_id) {
          navigate({
            to: "/dashboard/chess/play/$id",
            params: { id: String(data.game_id) },
          });
        } else {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/chess/lobby/create_lobby/`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      <div className="max-w-4xl mx-auto px-4 py-12">
        ...
        <div className="bg-card border border-border rounded-xl p-6 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            ...
            >
              <div className="text-4xl mb-2">Bot</div>
              <div className="font-semibold text-foreground mb-1">
                Play vs Bot
              </div>
              <div className="text-sm text-muted-foreground">
                No stakes, practice mode
              </div>
            </button>
            <button
              className={`p-6 rounded-xl border-2 transition-all ${
            ...
            >
              <div className="text-4xl mb-2">Human</div>
              <div className="font-semibold text-foreground mb-1">
                Play vs Human
              </div>
              <div className="text-sm text-muted-foreground">
                Play with others for stakes
              </div>
            </button>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-card border border-border rounded-xl p-4">
            ...
            <p className="text-sm text-muted-foreground">
              Standard chess rules apply. Win by checkmate or opponent
              resignation.
            </p>
          </div>
          <div className="bg-card border border-border rounded-xl p-4">
            ...
            <p className="text-sm text-muted-foreground">
              5% house rake on human games. Draw refunds both stakes. Winner
              takes all.
            </p>
          </div>
        </div>
